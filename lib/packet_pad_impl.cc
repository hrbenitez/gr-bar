...
//*
	if(tags.size()) {
		...
		dout << "found tag: " << pmt::symbol_to_string(t.key) << " : "<< nitems_written(0) << std::endl;

		uint64_t read = nitems_read(0);
		if(t.offset != read) {
			dout << "tag does not start at current offset, tag removed" << std::endl;
			n = std::min(n, t.offset - read);
			//remove_item_tag(0, t);

		} else {
			if(pmt::equal(t.key, pmt::mp("tx_sob")) || pmt::equal(t.key, pmt::mp("pdu_length"))) {
				dout << "tx_sob tag" << std::endl;
				int l = pmt::to_uint64(t.value);
				add_sob(nitems_written(0));
				d_pad = d_pad_front - l;     // originally d_pad = d_pad_front
				remove_item_tag(0, t);
				return 0;
...
//**/
  /** Tag-less Padding
  if (ninput == noutput) {    
    dout << "End of file detected" << std::endl;
    int check = nitems_written(0) % d_pad_tail;
    if (check) {
      d_pad = d_pad_tail - check;
    	if(d_pad < 0) d_pad = 0;	
    }
	}
  //*/
